...
public:
    ...
    uint64_t GuessCount(const PT &pt);
    uint64_t GuessCount(const WorkUnit &unit);
    vector<WorkUnit> SplitTask(const PT &pt, uint64_t max_guesses_per_unit);
    void GenerateRange(const WorkUnit &unit, vector<string> &output);
