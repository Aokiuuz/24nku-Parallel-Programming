...
void AppendSegmentValuesOpenMP(vector<string> &output, const vector<string> &values, int value_count)
{
...
#pragma omp parallel for schedule(dynamic, kOpenMpChunk) if (value_count > kOpenMpChunk)
    ...
    {
        output[base + i] = values[i];
    }
}

void AppendPrefixedSegmentValuesOpenMP(vector<string> &output, const string &prefix, const vector<string> &values, int value_count)
{
    const size_t base = output.size();
    output.resize(base + value_count);
