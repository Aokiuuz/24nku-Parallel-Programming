...
> `guess-pthread-advanced` 在本 Pthread 基础版之上继续加入了细粒度 profile 和优先队列优化，新增改动见本文最后一节。
...
细粒度测试显示，Pthread 8 线程下 `PriorityQueue::PopNext()` 中队列插入和队首删除占比较高。原始实现把 `priority` 维护为按概率降序排列的 `vector<PT>`：

- 新 PT 入队时从头线性扫描插入位置，复杂度为 $O(n)$。
- 队首 PT 出队时执行 `priority.erase(priority.begin())`，会搬移后续全部元素，复杂度也是 $O(n)$。

进阶版本把 `PCFG.h` 中的队列成员替换为 `std::priority_queue`：
...
struct QueuedPTCompare
{
    bool operator()(const QueuedPT &lhs, const QueuedPT &rhs) const
    {
        if (lhs.pt.prob != rhs.pt.prob)
        {
            return lhs.pt.prob < rhs.pt.prob;
        }
        return lhs.sequence > rhs.sequence;
    }
};

priority_queue<QueuedPT, vector<QueuedPT>, QueuedPTCompare> priority;
```

其中 `prob` 仍然作为主排序键，概率大的 PT 优先弹出；`sequence` 作为相同概率下的稳定次序，尽量保持与原始顺序一致。
...
```cpp
priority.push(QueuedPT{pt, queue_sequence});
queue_sequence += 1;
```
...
```cpp
PT current = priority.top().pt;
priority.pop();
```

之后对 `current` 执行原来的 `Generate(current)` 和 `current.NewPTs()`。新生成的 PT 不再线性扫描插入，而是直接：
...
因此队列维护从“线性扫描插入 + vector 队首删除”变为堆的 `push/pop`，理论复杂度从 $O(n)$ 降到 $O(\log n)$。本优化不改变 PCFG 生成逻辑、Pthread 生成逻辑、训练流程和 MD5 流程，只改变队列容器的维护方式。

为了便于重跑后分析，`main.cpp` 的 profile 输出也同步调整：

```text
Profile queue push count
Profile queue_push time
Profile queue_pop time
```
...
- `Profile queue_push time`
- `Profile queue_pop time`
- `Profile measured subtotal`
- `Guesses generated` 和 `Cracked` 是否保持一致
