...
inline __m512i bit_not_simd(__m512i x)
{
	return _mm512_xor_si512(x,_mm512_set1_epi32(0xFFFFFFFF));
}

inline __m512i ROTATELEFT_simd(__m512i num,int n)
{
    return _mm512_or_si512(_mm512_slli_epi32(num,n),_mm512_srli_epi32(num,32-n));
}

inline __m512i F_simd(__m512i x,__m512i y,__m512i z)
{
    return _mm512_or_si512(_mm512_and_si512(x,y),_mm512_and_si512(bit_not_simd(x),z));
}
inline __m512i G_simd(__m512i x,__m512i y,__m512i z)
{
    return _mm512_or_si512(_mm512_and_si512(x,z),_mm512_and_si512(y,bit_not_simd(z)));
}
inline __m512i H_simd(__m512i x,__m512i y,__m512i z)
{
    return _mm512_xor_si512(_mm512_xor_si512(x,y),z);
}
inline __m512i I_simd(__m512i x,__m512i y,__m512i z)
{
    return _mm512_xor_si512(y,_mm512_or_si512(x,bit_not_simd(z)));
}

inline void FF_simd(__m512i *a,__m512i b,__m512i c,__m512i d,__m512i x,int s,uint32_t ac)
{
    __m512i ac_vec = _mm512_set1_epi32(ac);
    *a = _mm512_add_epi32(*a,_mm512_add_epi32(F_simd(b,c,d),_mm512_add_epi32(x,ac_vec)));
    *a = ROTATELEFT_simd(*a,s);
    *a = _mm512_add_epi32(*a,b);
}
inline void GG_simd(__m512i *a,__m512i b,__m512i c,__m512i d,__m512i x,int s,uint32_t ac)
{
    __m512i ac_vec = _mm512_set1_epi32(ac);
    *a = _mm512_add_epi32(*a,_mm512_add_epi32(G_simd(b,c,d),_mm512_add_epi32(x,ac_vec)));
    *a = ROTATELEFT_simd(*a,s);
    *a = _mm512_add_epi32(*a,b);
}
inline void HH_simd(__m512i *a,__m512i b,__m512i c,__m512i d,__m512i x,int s,uint32_t ac)
{
    __m512i ac_vec = _mm512_set1_epi32(ac);
    *a = _mm512_add_epi32(*a,_mm512_add_epi32(H_simd(b,c,d),_mm512_add_epi32(x,ac_vec)));
    *a = ROTATELEFT_simd(*a,s);
    *a = _mm512_add_epi32(*a,b);
}
inline void II_simd(__m512i *a,__m512i b,__m512i c,__m512i d,__m512i x,int s,uint32_t ac)
{
    __m512i ac_vec = _mm512_set1_epi32(ac);
    *a = _mm512_add_epi32(*a,_mm512_add_epi32(I_simd(b,c,d),_mm512_add_epi32(x,ac_vec)));
    *a = ROTATELEFT_simd(*a,s);
    *a = _mm512_add_epi32(*a,b);
}
...
void MD5Hash_SIMD(string input[16],__m512i state[4])
{
    ...
    for(int i = 0;i < n_block;i++)
    {
        ...
        for(int i1 = 0;i1 < 16;i1++)
        {
            ...
            x[i1] = _mm512_loadu_si512((__m512i*)y);
        }
       	__m512i a,b,c,d;
        a = state[0];
        b = state[1];
    ...
    for(int i = 0;i < 4;i++)
    {
        __m512i value = state[i];
        __m512i tmp1 = _mm512_set1_epi32(0xff);
        __m512i tmp2 = _mm512_set1_epi32(0xff00);
        __m512i tmp3 = _mm512_set1_epi32(0xff0000);
        __m512i tmp4 = _mm512_set1_epi32(0xff000000);
        state[i] = _mm512_or_si512(_mm512_or_si512(_mm512_slli_epi32(_mm512_and_si512(value,tmp1),24),
    	_mm512_slli_epi32(_mm512_and_si512(value,tmp2),8)),
        _mm512_or_si512(_mm512_srli_epi32(_mm512_and_si512(value,tmp3),8),_mm512_srli_epi32(_mm512_and_si512(value,tmp4),24)));
    }
    for(int i = 0; i < 4; i++) 
    {
