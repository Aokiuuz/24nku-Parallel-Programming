...
inline __m256i bit_not_simd(__m256i x)
{
...
{
    return _mm256_or_si256(_mm256_and_si256(x,y),_mm256_and_si256(bit_not_simd(x),z));
}
inline __m256i G_simd(__m256i x,__m256i y,__m256i z)
{
    return _mm256_or_si256(_mm256_and_si256(x,z),_mm256_and_si256(y,bit_not_simd(z)));
}
...
{
    return _mm256_xor_si256(y,_mm256_or_si256(x,bit_not_simd(z)));
}

inline void FF_simd(__m256i *a,__m256i b,__m256i c,__m256i d,__m256i x,int s,uint32_t ac)
{
    __m256i ac_vec = _mm256_set1_epi32(ac);
    *a = _mm256_add_epi32(*a,_mm256_add_epi32(F_simd(b,c,d),_mm256_add_epi32(x,ac_vec)));
    *a = ROTATELEFT_simd(*a,s);
    *a = _mm256_add_epi32(*a,b);
}
inline void GG_simd(__m256i *a,__m256i b,__m256i c,__m256i d,__m256i x,int s,uint32_t ac)
{
    __m256i ac_vec = _mm256_set1_epi32(ac);
    *a = _mm256_add_epi32(*a,_mm256_add_epi32(G_simd(b,c,d),_mm256_add_epi32(x,ac_vec)));
    *a = ROTATELEFT_simd(*a,s);
    *a = _mm256_add_epi32(*a,b);
}
inline void HH_simd(__m256i *a,__m256i b,__m256i c,__m256i d,__m256i x,int s,uint32_t ac)
{
    __m256i ac_vec = _mm256_set1_epi32(ac);
    *a = _mm256_add_epi32(*a,_mm256_add_epi32(H_simd(b,c,d),_mm256_add_epi32(x,ac_vec)));
    *a = ROTATELEFT_simd(*a,s);
    *a = _mm256_add_epi32(*a,b);
}
inline void II_simd(__m256i *a,__m256i b,__m256i c,__m256i d,__m256i x,int s,uint32_t ac)
{
    __m256i ac_vec = _mm256_set1_epi32(ac);
    *a = _mm256_add_epi32(*a,_mm256_add_epi32(I_simd(b,c,d),_mm256_add_epi32(x,ac_vec)));
    *a = ROTATELEFT_simd(*a,s);
    *a = _mm256_add_epi32(*a,b);
}

void MD5Hash_SIMD(string input[8],__m256i state[4])
{
    ...
    for(int i = 0;i < 4;i++)
    {
        ...
        __m256i tmp1 = _mm256_set1_epi32(0xff);
        __m256i tmp2 = _mm256_set1_epi32(0xff00);
        __m256i tmp3 = _mm256_set1_epi32(0xff0000);
        __m256i tmp4 = _mm256_set1_epi32(0xff000000);
        state[i] = _mm256_or_si256(_mm256_or_si256(_mm256_slli_epi32(_mm256_and_si256(value,tmp1),24),
    	_mm256_slli_epi32(_mm256_and_si256(value,tmp2),8)),
        _mm256_or_si256(_mm256_srli_epi32(_mm256_and_si256(value,tmp3),8),_mm256_srli_epi32(_mm256_and_si256(value,tmp4),24)));
    }
    for(int i = 0; i < 4; i++) 
    {
