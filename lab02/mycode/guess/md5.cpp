...
static bit32 DecodeWord(const Byte *message, int offset)
{
...
{
	return vorrq_u32(vshlq_u32(num, vdupq_n_s32(n)),
...
void MD5Hash_simdversion(string input[4], uint32x4_t state[4])
{
	...
	for (int i = 0; i < n_blocks; i += 1)
	{
		...
		for (int i1 = 0; i1 < 16; ++i1)
		{
			...
			{
				lanes[lane] = DecodeWord(paddedMessage[lane], 4 * i1 + i * 64);
			}
			x[i1] = vld1q_u32(lanes);
		}
	...
	for (int i = 0; i < 4; i++)
	{
		...
		state[i] = vorrq_u32(
			vorrq_u32(vshlq_n_u32(vandq_u32(value, vdupq_n_u32(0xff)), 24),
					  vshlq_n_u32(vandq_u32(value, vdupq_n_u32(0xff00)), 8)),
			vorrq_u32(vshrq_n_u32(vandq_u32(value, vdupq_n_u32(0xff0000)), 8),
					  vshrq_n_u32(vandq_u32(value, vdupq_n_u32(0xff000000)), 24)));
	}
